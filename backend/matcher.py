...
def career_to_vector(career_vector: List[float]) -> np.ndarray:
...
def get_top_careers(state: Dict[str, float]) -> List[Dict[str, float]]:
    ...
    user_vector = state_to_vector(state)
    user_vector = user_vector - 0.5  # center: uncertain traits contribute 0, high +, low -
    scored_careers: List[Dict[str, float]] = []

    for career in careers:
        career_vector = career_to_vector(career["vector"])
        score = cosine_similarity(user_vector, career_vector)
